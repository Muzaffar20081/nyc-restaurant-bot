...
# Простая загрузка меню
def load_menu(cafe_key):
    """Загружает меню для конкретного кафе - УПРОЩЕННАЯ ВЕРСИЯ"""
    try:
        ...
        logger.info(f"Путь к модулю меню: {module_path}")
        
        # Упрощенный импорт меню
        if module_path == "menus.italian_menu":
            from menus import italian_menu
            menu_module = italian_menu
        elif module_path == "menus.sushi_menu":
            from menus import sushi_menu
            menu_module = sushi_menu
        elif module_path == "menus.burger_menu":
            from menus import burger_menu
            menu_module = burger_menu
        else:
            # Пробуем обычный импорт
            menu_module = importlib.import_module(module_path)
        
        logger.info(f"Меню загружено: {len(menu_module.CATEGORIES)} категорий, {len(menu_module.ALL_ITEMS)} товаров")
        ...
        # Возвращаем простые тестовые данные при ошибке
        if cafe_key == "italy":
            CATEGORIES = {
...
async def show_categories(call: types.CallbackQuery):
    ...
    try:
        ...
        for category_name in CATEGORIES.keys():
            ...
            callback_data = f"category_{category_name[:30].replace(' ', '_')}"
