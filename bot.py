...
def create_main_menu_keyboard():
    """Создает клавиатуру главного меню с категориями"""
    keyboard = []
...
async def ask_ai(query: str) -> str:
    ...
    search_words = ["найти", "поиск", "искать", "есть ли", "хочу"]
    if any(word in query_lower for word in search_words):
        # Убираем слова поиска из запроса
        ...
        # Ищем товары во всех категориях
        ...
        if results:
            ...
            response += "\n".join(results[:5])  # Ограничиваем 5 результатами
            if len(results) > 5:
                response += f"\n\n...и еще {len(results) - 5} товаров"
    ...
    faq = {
        ...
        "доставка": "🚚 Доставка занимает 30-60 минут. Бесплатная от 1000₽.",
        ...
        "помощь": "🤖 Используй кнопки меню для навигации. Пиши название блюда для поиска.",
        "привет": "👋 Привет! Выбери категорию из меню или напиши, что ищешь.",
        "спасибо": "😊 Пожалуйста! Приятного аппетита!",
        "меню": "📋 Используй кнопку 'Меню' или напиши 'меню' для просмотра категорий."
    }
    ...
    # Если не нашли в FAQ, ищем похожие товары
    if len(query_lower) > 2:
        ...
        for category in MENU_CATEGORIES:
            ...
            for item in items:
                ...
                if query_lower in item_name_lower or any(word in item_name_lower for word in query_lower.split() if len(word) > 2):
                    results.append(f"• {item['name']} - {item['price']}₽")
        
        if results:
            response = "🔍 *Возможно, ты искал:*\n\n"
            response += "\n".join(results[:3])
            response += "\n\nИспользуй кнопки меню для подробностей!"
            return response
    
    # Стандартный ответ
    return "🤖 Используй кнопки меню для выбора товаров или напиши название блюда для поиска."
...
async def add_item(call: types.CallbackQuery):
    ...
    # Возвращаемся к категории товара
...
async def checkout(call: types.CallbackQuery):
    ...
    caption += "Спасибо за заказ! Приятного аппетита! 😊"
...
async def text_handler(message: types.Message):
    ...
    if text in ["меню", "menu", "категории"]:
        # Имитируем нажатие кнопки "назад к категориям"
    ...
    if text in ["корзина", "cart", "заказ"]:
        # Имитируем нажатие кнопки "корзина"
        call = types.CallbackQuery(
            id="0",
            from_user=message.from_user,
            chat_instance="0",
    ...
    if text in ["очистить корзину", "очистить"]:
        # Имитируем нажатие кнопки "очистить корзину"
        call = types.CallbackQuery(
            id="0",
            from_user=message.from_user,
            chat_instance="0",
        ...
        await clear_cart(call)
        return
    
    # Обработка текстовых запросов через AI
